...
stepLoop (Loop e1 e2 e3) = 
    case stepLoop e1 of
        ...
        Nothing ->
            case e1 of
                Zero -> Just e2
                PlusOne e1 -> Just (App e3 (Loop e1 e2 e3))
                _ -> Nothing

-- Representations of the number 83
twoDigits :: Int -> Expr
twoDigits n = iterate PlusOne Zero !! n
...
{- [Question 2.2]:
   No, it is not possible to extend our language implementation to include this reduction rule as adding this rule would lead 
   to uncertainty in the reduction process since we have not defined any such rule that states that expressions get evaluated 
   from left to right. 

   For example, consider the expression stepLoop(Loop (App (Lam "x" (Var "x")) (Var "a")) (App (Lam "y" (Var "y")) (Var "b")) (Var "c"))

   We could either follow the first reduction rule or the additional reduction rule and there is no way to determine which path to choose.
   This results in our program being non deterministic which is undesirable. Hence, we cannot extend our Loopy Lambda implementation to
   include this additional reduction rule.
-}
